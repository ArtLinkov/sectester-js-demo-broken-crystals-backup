...
In the example above, the CI flow is set to run all the SecTester tests, but if we would like to run a single test we can update the yml file by specifying a specific file such as `testimonials.e2e-spec.ts` (which in our case has a single test), as follows:

```yaml
- name: Run SecTester
  run: npm run test:sec -- testimonials.e2e-spec.ts
  env:
    BRIGHT_TOKEN: ${{ secrets.BRIGHT_TOKEN }}
    BRIGHT_HOSTNAME: app.neuralegion.com
    BROKEN_CRYSTALS_URL: http://localhost:3000
